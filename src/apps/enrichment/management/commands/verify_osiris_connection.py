...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--course", type=int, help="Course code to test", default=191154340
        )  # Example from legacy
        parser.add_argument(
            "--person", type=str, help="Person name to test", default="Augustijn"
        )
    ...
    def handle(self, *args, **options):
        ...
        async def run_test():
            async with OsirisScraperService() as scraper:
                ...
                if course_data:
                    self.stdout.write(
                        self.style.SUCCESS(f"Course found: {course_data.get('name')}")
                    )
                    self.stdout.write(
                        f"  Internal ID: {course_data.get('internal_id')}"
                    )
                    self.stdout.write(f"  Teachers: {course_data.get('teachers')}")
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Course {course_code} not found or error occurred"
                        )
                    )
                ...
                if person_data:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Person found: {person_data.get('main_name')}"
                        )
                    )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Person {person_name} not found or error occurred"
                        )
                    )
