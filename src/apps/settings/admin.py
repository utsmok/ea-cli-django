...
class SettingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("key", "name", "description", "category")}),
        (
            "Value Configuration",
            {"fields": ("value_type", "value", "default_value", "choices")},
        ),
        ("Validation", {"fields": ("is_required",), "classes": ("collapse",)}),
        (
            "Security",
            {
                "fields": ("is_sensitive",),
...
def export_yaml(modeladmin, request, queryset):
    ...
    yaml_content = yaml.dump(settings_dict, default_flow_style=False, sort_keys=False)
