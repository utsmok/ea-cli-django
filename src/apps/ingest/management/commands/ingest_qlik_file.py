...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("file_path", type=str, help="The absolute or relative path to the Qlik Excel file.")
    ...
    def handle(self, *args, **options):
        ...
        if not stage_result["success"]:
            self.stderr.write(self.style.ERROR(f"Staging failed: {batch.error_message}"))
            return
        
        self.stdout.write(self.style.SUCCESS(f"Staging complete. {batch.rows_staged} rows staged."))

        self.stdout.write("Processing batch...")
        process_result = process_batch(batch.id)
        if not process_result["success"]:
            self.stderr.write(self.style.ERROR(f"Processing failed: {batch.error_message}"))
            return
